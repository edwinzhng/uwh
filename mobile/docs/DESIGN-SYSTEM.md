...
`AppLayout` owns the responsive sidebar and safe-area spacing, with club branding at the top left and notifications/profile switching at the right. Detail pages place a content-width ghost back button above the title, aligning its arrow with the title’s leading edge. Settings lives in the profile menu, without a separate navigation button. A root `NavigationFrame` keeps the mobile tab bar mounted across route changes. Feature code composes `Surface` and `ListItem` through the public API. Existing button/field/segment geometry remains unchanged.
