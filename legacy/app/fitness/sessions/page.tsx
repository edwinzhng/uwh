...
export default function FitnessPage(): React.JSX.Element {
	...
	// Stale-while-revalidating: keep last known workspace visible during test switches
	// so the content never flashes away while the new workspace query is in-flight.
	// Effects below still use `workspace` (real query result) for correctness.
	const stableWorkspaceRef = useRef<FitnessWorkspace | undefined>(undefined);
	if (workspace !== undefined) {
		stableWorkspaceRef.current = workspace;
	}
	const stableWorkspace = workspace ?? stableWorkspaceRef.current;
	const loading = loadingTests || loadingPlayers;
	...
	const bestResultsByPlayerId = useMemo(
		(): Record<string, string> =>
			Object.fromEntries(
				(stableWorkspace?.bestResults ?? []).map((r) => [r.playerId, r.value]),
			),
		[stableWorkspace?.bestResults],
	);
	...
	const selectedSession = stableWorkspace?.selectedSession;
	...
	const handleSessionChange = (sessionValue: string): void => {
		const nextSession = stableWorkspace?.sessions.find(
			...
			<PageHeader
				...
				actions={
					<div className="flex items-center gap-2">
						<Link
							href="/fitness"
							className="flex h-9 flex-none items-center gap-1.5 border border-[#cbdbcc] px-4 text-xs font-semibold tracking-[0.08em] uppercase text-[#021e00] hover:border-[#021e00] hover:bg-white whitespace-nowrap"
						>
						...
						<div>
											...
											<div className="relative mt-2">
												...
												{isSessionMenuOpen ? (
													<div className="absolute left-0 top-full z-20 mt-1 min-w-[180px] border border-[#cbdbcc] bg-white shadow-lg">
														{stableWorkspace.sessions.length === 0 ? (
															<div className="px-4 py-2.5 text-sm text-[#6c866d]">
																No sessions yet
															</div>
														) : (
															stableWorkspace.sessions.map((session) => (
																<button
																	key={session._id}
																	type="button"
																	onClick={() =>
							...
							<div className="flex-1 px-7 py-5">
								...
								<div className="mt-7 grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-3">
									{stableWorkspace.summaryCards.map((card) => (
										<div
											key={card.label}
				...
				<EditFitnessTestSessionModal
					...
					onDeleted={() => {
						const currentSessionIndex =
							stableWorkspace?.sessions.findIndex(
								(session) => session._id === selectedSessionId,
							) ?? -1;
						const nextSession = stableWorkspace?.sessions.at(
							currentSessionIndex + 1,
						);
						const previousSession =
							currentSessionIndex > 0
								? stableWorkspace?.sessions.at(currentSessionIndex - 1)
								: undefined;
